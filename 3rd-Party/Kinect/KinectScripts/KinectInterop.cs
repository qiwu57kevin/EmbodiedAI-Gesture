...
//						}
	...
	public static string GetMatrixAsCsv(ref Matrix4x4 mat, char delimiter)
	{
        ...
        for (int i = 0; i < 4; i++)
		{
			...
			sbBuf.AppendFormat(invCulture, "{0:F3}", vRow.x).Append(delimiter);
			sbBuf.AppendFormat(invCulture, "{0:F3}", vRow.y).Append(delimiter);
			sbBuf.AppendFormat(invCulture, "{0:F3}", vRow.z).Append(delimiter);
			sbBuf.AppendFormat(invCulture, "{0:F3}", vRow.w).Append(delimiter);
		}
	...
	public static bool SetMatrixFromCsv(string sCsvLine, ref Matrix4x4 mat)
	{
		...
		for(int i = 0; i < 4; i++)
		{
			...
			float.TryParse(alCsvParts[iIndex + 3], numFloat, invCulture, out w);
			iIndex += 4;
        ...
        for (int i = 0; i < sensorData.bodyCount; i++)
		{
			...
			if(bodyFrame.bodyData[i].bIsTracked != 0)
			{
				...
				for(int j = 0; j < sensorData.jointCount; j++)
				{
					...
					{
						sbBuf.AppendFormat(invCulture, "{0:F3}", jointData.kinectPos.x).Append(delimiter);
						sbBuf.AppendFormat(invCulture, "{0:F3}", jointData.kinectPos.y).Append(delimiter);
						sbBuf.AppendFormat(invCulture, "{0:F3}", jointData.kinectPos.z).Append(delimiter);
					}
				}
			}
		}
				...
				if(sensorData.color2DepthCoords != null)
				{
					...
					lock(sensorData.depthCoordsBufferLock)
					{
					    ...
					    (sensorData.sensorInterface.IsBackgroundRemovalActive() && 
						 sensorData.sensorInterface.GetSensorPlatform() != DepthSensorPlatform.KinectSDKv1)))
				{
					//// wait for buffer release
					//while(sensorData.depthCoordsBufferReady)
					//{
