...
using System;
using System.IO;
using nuitrack.facedata;


public class NuitrackInterface : DepthSensorInterface 
{

    // public class used to serialize & deserialize the face landmarks
    [Serializable]
...
//	};
	...
	private Instances[] faceInstances;
    private FaceLandmarks faceLandmarks = new FaceLandmarks();
...
//			sensorData.colorImageHeight = colorSensor != null ? colorSensor.GetOutputMode().YRes : 480;
			sensorData.colorImageWidth = colorFrame != null ? colorFrame.Cols : 640;
			sensorData.colorImageHeight = colorFrame != null ? colorFrame.Rows : 480;
...
//		sensorData.depthImageHeight = depthSensor != null ? depthSensor.GetOutputMode().YRes : 480;
		sensorData.depthImageWidth = depthFrame != null ? depthFrame.Cols : 640;
		sensorData.depthImageHeight = depthFrame != null ? depthFrame.Rows : 480;
...
//	}
	...
	public bool UpdateFaceTracking ()
	{
		if (bFaceTrackingInited) 
		{
			...
			faceInstances = faceInfo.Instances;
			if (faceInstances != null) 
			{
				for (int i = 0; i < faceInstances.Length; i++)
				{
					// set head-pos and head-rot
					nuitrack.facedata.Angles headAngles = faceInstances[i].face.angles;

					if (headAngles != null) 
					{
						faceInstances[i].face.headRot = Quaternion.Euler(headAngles.pitch, -headAngles.yaw, -headAngles.roll);
						faceInstances[i].face.headPos = KinectManager.Instance.GetJointKinectPosition(faceInstances[i].id, (int)KinectInterop.JointType.Head);
					}
				}
			}
		}
	...
	public bool IsFaceTracked (long userId)
	{
		...
		for (int i = 0; i < faceInstances.Length; i++)
		{
			if(faceInstances[i] != null && faceInstances[i].id == (int)userId && faceInstances[i].face != null)
			{
				return true;
			}
		}
	...
	{
		if (faceInstances == null || colorFrame == null)
			return false;

		for (int i = 0; i < faceInstances.Length; i++)
		{
			if(faceInstances[i] != null && faceInstances[i].id == (int)userId)
			{
				if(faceInstances[i].face != null && faceInstances[i].face.rectangle != null)
				{
					var faceBox = faceInstances[i].face.rectangle;

					faceRect.x = faceBox.left * (float)colorFrame.Cols;
					faceRect.y = faceBox.top * (float)colorFrame.Rows;
					faceRect.width = faceBox.width * (float)colorFrame.Cols;
					faceRect.height = faceBox.height * (float)colorFrame.Rows;
	...
	{
		if(bFaceTrackingInited && faceInstances != null && texColor != null)
		{
			for (int i = 0; i < faceInstances.Length; i++)
			{
				if(faceInstances[i] != null && faceInstances[i].face != null && faceInstances[i].face.rectangle != null)
				{
					var faceBox = faceInstances[i].face.rectangle;
					Rect faceRect = new Rect (faceBox.left * texColor.width, faceBox.top * texColor.height, faceBox.width * texColor.width, faceBox.height * texColor.height);

					UnityEngine.Color color = UnityEngine.Color.magenta;
					Vector2 pt1, pt2;

					// bottom
					pt1.x = faceRect.xMin; pt1.y = faceRect.yMin;
					pt2.x = faceRect.xMax; pt2.y = pt1.y;
					DrawLine(texColor, pt1, pt2, color);

					// right
					pt1.x = pt2.x; pt1.y = pt2.y;
					pt2.x = pt1.x; pt2.y = faceRect.yMax;
					DrawLine(texColor, pt1, pt2, color);

					// top
					pt1.x = pt2.x; pt1.y = pt2.y;
					pt2.x = faceRect.xMin; pt2.y = pt1.y;
					DrawLine(texColor, pt1, pt2, color);

					// left
					pt1.x = pt2.x; pt1.y = pt2.y;
					pt2.x = pt1.x; pt2.y = faceRect.yMin;
					DrawLine(texColor, pt1, pt2, color);
				}
			}
		}
	...
	public bool GetHeadPosition (long userId, ref Vector3 headPos)
	{
		...
		for (int i = 0; i < faceInstances.Length; i++)
		{
			if(faceInstances[i] != null && faceInstances[i].id == (int)userId && faceInstances[i].face != null)
			{
				headPos = faceInstances[i].face.headPos;
				return true;
			}
		}
	...
	public bool GetHeadRotation (long userId, ref Quaternion headRot)
	{
		...
		for (int i = 0; i < faceInstances.Length; i++)
		{
			if(faceInstances[i] != null && faceInstances[i].id == (int)userId && faceInstances[i].face != null)
			{
				headRot = faceInstances[i].face.headRot;
				return true;
			}
		}
	...
	public bool GetFaceProperties(long userId, ref Dictionary<string, string> faceProps)
	{
		...
		for (int i = 0; i < faceInstances.Length; i++)
		{
			if(faceInstances[i] != null && faceInstances[i].id == (int)userId && faceInstances[i].face != null)
			{
				if (faceInstances [i].face.gender != null) 
				{
					faceProps["gender"] = faceInstances[i].face.gender;
				}

				if (faceInstances[i].face.age != null) 
				{
					faceProps["age"] = faceInstances[i].face.age.years.ToString();
					faceProps["agetype"] = faceInstances[i].face.age.type;
				}

                if(faceInstances[i].face.landmark != null)
                {
                    faceLandmarks.landmark = faceInstances[i].face.landmark;
                    faceProps["landmarks"] = JsonUtility.ToJson(faceLandmarks);
                }

                //if(faceInstances[i].face.left_eye != null)
                {
                    faceProps["lefteye"] = JsonUtility.ToJson(faceInstances[i].face.left_eye);
                }

                //if (faceInstances[i].face.right_eye != null)
                {
                    faceProps["righteye"] = JsonUtility.ToJson(faceInstances[i].face.right_eye);
                }
                
                nuitrack.facedata.Emotions emots = faceInstances[i].face.emotions;

				if (emots != null) 
				{
					faceProps["neutral"] = emots.neutral.ToString();
					faceProps["angry"] = emots.angry.ToString();
					faceProps["surprise"] = emots.surprise.ToString();
					faceProps["happy"] = emots.happy.ToString();

					string emotMax = "neutral";
					float emotVal = emots.neutral;

					if (emotVal < emots.angry) 
					{
						emotMax = "angry";
						emotVal = emots.angry;
					}
					if (emotVal < emots.surprise) 
					{
						emotMax = "surprise";
						emotVal = emots.surprise;
					}
					if (emotVal < emots.happy) 
					{
						emotMax = "happy";
						emotVal = emots.happy;
					}

					faceProps["emotion"] = emotMax;
				}
	...
	public int GetFaceModelVerticesCount (long userId)
	{
        ...
        for (int i = 0; i < faceInstances.Length; i++)
        {
            if (faceInstances[i] != null && (userId == 0 || faceInstances[i].id == (int)userId) && 
                faceInstances[i].face != null && faceInstances[i].face.landmark != null)
            {
                return faceInstances[i].face.landmark.Length;
            }
        }
	...
	public bool GetFaceModelVertices (long userId, ref Vector3[] avVertices)
	{
        ...
        for (int i = 0; i < faceInstances.Length; i++)
        {
            if (faceInstances[i] != null && (userId == 0 || faceInstances[i].id == (int)userId) && 
                faceInstances[i].face != null && faceInstances[i].face.landmark != null)
            {
                int verticesCount = faceInstances[i].face.landmark.Length;

                //Matrix4x4 matHeadRot = Matrix4x4.identity;
                //matHeadRot.SetTRS(Vector3.zero, faceInstances[i].face.headRot, Vector3.one);

                if(avVertices != null && avVertices.Length == verticesCount && depthFrame != null)
                {
                    for (int v = 0; v < verticesCount; v++)
                    {
                        float dx = faceInstances[i].face.landmark[v].x;
                        int iDx = Mathf.Clamp(Mathf.RoundToInt((dontHFlipFrame ? dx : (1f - dx)) * depthFrame.Cols), 0, depthFrame.Cols - 1);

                        float dy = faceInstances[i].face.landmark[v].y;
                        int iDy = Mathf.Clamp(Mathf.RoundToInt(dy * depthFrame.Rows), 0, depthFrame.Rows - 1);

                        int dIndex = iDy * depthFrame.Cols + iDx;
                        ushort depthVal = depthFrame[dIndex];

                        Vector2 depthPos = new Vector2(iDx, iDy);
                        avVertices[v] = coordMapper != null ? coordMapper.MapDepthPointToSpaceCoords(depthPos, depthVal) : Vector3.zero;

                        //avVertices[v] = matHeadRot.MultiplyPoint3x4(avVertices[v]);
                    }
                }
    ...
    // NuiTrack face model indices
    // see: http://download.3divi.com/Nuitrack/doc/Instance_based_API.html
    private static readonly int[] nuitrackFaceIndices =
    {
        0, 1, 2,  // left eyebrow
        3, 4, 5,  // right eyebrow
        8, 6, 7, 7, 6, 10, 8, 7, 10, 8, 10, 9,  // nose
        11, 12, 13, 11, 13, 14, 11, 14, 15, 11, 15, 16,  // left eye
        17, 18, 19, 17, 19, 20, 17, 20, 21, 17, 21, 22,  // right eye
        23, 24, 25, 23, 25, 26  // mouth
    };

	public int GetFaceModelTrianglesCount ()
	{
		return nuitrackFaceIndices.Length;
	}

	public bool GetFaceModelTriangles (bool bMirrored, ref int[] avTriangles)
	{
        Array.Copy(nuitrackFaceIndices, avTriangles, nuitrackFaceIndices.Length);
		return true;
	}
